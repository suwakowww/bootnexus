...
if ($action == "viewtopic")
{
	...
	for ($i = 0; $i < $pages; ++$i)
	{
		...
		$dotted = 0;
		if ($i != $page)
		$pagerarr[] .= "<li><a href=\"".htmlspecialchars("?".$addparam."&page=".$i)."\">".($i+1)."</a></li>\n";
		else
		$pagerarr[] .= "<li class=\"active\"><a href=\"#\">".($i+1)."</a></li>\n";
	}
	if ($page == 0)
	$pager = "<li class=\"previous disabled\"><a href=\"#\">&larr;".$lang_forums['text_prev']."</a></li>";
	else
	$pager = "<li class=\"previous\"><a href=\"".htmlspecialchars("?".$addparam."&page=" . ($page - 1)) .
	"\">&larr;".$lang_forums['text_prev']."</a></li>";
	if ($page == $pages-1)
	$pager .= "<li class=\"next disabled\"><a href=\"#\">".$lang_forums['text_next']." &rarr;</a></li>\n";
	else
	$pager .= "<li class=\"next\"><a href=\"".htmlspecialchars("?".$addparam."&page=" . ($page + 1)) .
	...
	while ($arr = mysql_fetch_assoc($res))
	{
		...
		print("<div style=\"margin-top: 8pt; margin-bottom: 8pt;\"><table id=\"pid".$postid."\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" width=\"100%\"><tr><td class=\"embedded\"><a href=\"".htmlspecialchars("forums.php?action=viewtopic&topicid=".$topicid."&page=p".$postid."#pid".$postid)."\">#".$postid."</a>&nbsp;&nbsp;<font color=\"gray\">".$lang_forums['text_by']."</font>".$by."&nbsp;&nbsp;<font color=\"gray\">".$lang_forums['text_at']."</font>".$added);
		...
		print("</td><td class=\"text-right\"><span>".$lang_forums['text_number']."<b>".($pn+$offset)."</b>".$lang_forums['text_lou']."&nbsp;&nbsp;</span><a href=\"#top\" title=\"".$lang_forums['text_back_to_top']."\"><span class=\"glyphicon glyphicon-plane\"></span></a></td></tr>");
		...
		print("<tr><td class=\"rowfollow\"><div class=\"btn-group btn-group-xs\">".("'".$arr2['last_access']."'">$dt?"<a href=\"#\" class=\"btn btn-success\" disabled>".$lang_forums['title_online']."</a>":"<a href=\"#\" class=\"btn btn-default\" disabled>".$lang_forums['title_offline']."</a>" )."<a class=\"btn btn-info\" href=\"sendmessage.php?receiver=".htmlspecialchars(trim($arr2["id"]))."\" title=\"".$lang_forums['title_send_message_to'].htmlspecialchars($arr2["username"])."\">PM</a><a href=\"report.php?forumpost=$postid\" class=\"btn btn-danger\" title=\"".$lang_forums['title_report_this_post']."\"><span class=\"glyphicon glyphicon-exclamation-sign\"></span></a></div></td>");
		print("<td class=\"text-right\"><div class=\"btn-group btn-group-xs\">");

		if ($maypost)
		print("<a class=\"btn btn-primary\" href=\"".htmlspecialchars("?action=quotepost&postid=".$postid)."\">".$lang_forums['title_reply_with_quote']."</a>");

		if (get_user_class() >= $postmanage_class || $is_forummod)
		print("<a class=\"btn btn-primary\" href=\"".htmlspecialchars("?action=deletepost&postid=".$postid)."\">".$lang_forums['title_delete_post']."</a>");

		if (($CURUSER["id"] == $posterid && !$locked) || get_user_class() >= $postmanage_class || $is_forummod)
		print("<a class=\"btn btn-primary\" href=\"".htmlspecialchars("?action=editpost&postid=".$postid)."\">".$lang_forums['title_edit_post']."</a>");
	...
	if (get_user_class() >= $postmanage_class || $is_forummod)
	{
		...
		print("<input type=\"hidden\" name=\"sticky\" value=\"".($sticky ? 'no' : 'yes')."\" /><input type=\"submit\" class=\"btn btn-info\" value=\"".($sticky ? $lang_forums['submit_unsticky'] : $lang_forums['submit_sticky'])."\" />&nbsp;</form></td>\n");
		print("<td class=\"embedded\"><form method=\"post\" action=\"?action=setlocked\">\n");
		print("<input type=\"hidden\" name=\"topicid\" value=\"".$topicid."\" />\n");
		print("<input type=\"hidden\" name=\"returnto\" value=\"".htmlspecialchars($_SERVER[REQUEST_URI])."\" />\n");
		print("<input type=\"hidden\" name=\"locked\" value=\"".($locked ? 'no' : 'yes')."\" /><input type=\"submit\" class=\"btn btn-warning\" value=\"".($locked ? $lang_forums['submit_unlock'] : $lang_forums['submit_lock'])."\" />&nbsp;</form></td>\n");
		...
		print("<input type=\"submit\" class=\"btn btn-warning\" value=\"".$lang_forums['submit_delete_topic']."\" />&nbsp;</form></td>\n");
		print("<td class=\"embedded\"><form class=\"form form-inline\" method=\"post\" action=\"".htmlspecialchars("?action=movetopic&topicid=".$topicid)."\">\n"."&nbsp;".$lang_forums['text_move_thread_to']."&nbsp;<select class=\"form-control\" name=\"forumid\">");
		...
		print("</select> <input type=\"submit\" class=\"btn btn-info\" value=\"".$lang_forums['submit_move']."\" /></form></td>");
		print("<td class=\"embedded\"><form class=\"form form-inline\" method=\"post\" action=\"".htmlspecialchars("?action=hltopic&topicid=".$topicid)."\">\n"."&nbsp;".$lang_forums['text_highlight_topic']."&nbsp;<select class=\"form-control\" name=\"color\">");
...
<option style='background-color: white' value=\"40\">White</option>");
		print("</select>");
		print("<input type=\"hidden\" name=\"returnto\" value=\"".htmlspecialchars($_SERVER[REQUEST_URI])."\" />\n");
		print("<input type=\"submit\" class=\"btn btn-info\" value=\"".$lang_forums['submit_change']."\" /></form></td>");
		print("</tr>\n");
		print("</table>\n");
	}
	...
	print($pagerbottom);
	if ($maypost){
	print("<br /><div class=\"container\"><table><tr>".
...
if ($action == "viewforum")
{
	...
	print("</td><td class=\"text-right\">");
	if ($maypost)
		print("<a class=\"btn btn-primary\" href=\"".htmlspecialchars("?action=newtopic&forumid=".$forumid)."\">".$lang_forums['title_new_topic']."</a>&nbsp;&nbsp;");
	print("</td>");
	print("</tr></table>\n");
	if ($numtopics > 0)
	{
		...
		print("<thead><tr><th style=\"width:70%;\">".$lang_forums['col_topic']."</th><th class=\"text-center\"><a href=\"".htmlspecialchars("?action=viewforum&forumid=".$forumid.$addparam."&sort=".($_GET["sort"] == 'firstpostdesc' ? "firstpostasc" : "firstpostdesc"))."\" title=\"".($_GET["sort"] == 'firstpostdesc' ?  $lang_forums['title_order_topic_asc'] : $lang_forums['title_order_topic_desc'])."\">".$lang_forums['col_author']."</a></th><th class=\"text-center\">".$lang_forums['col_replies']."/".$lang_forums['col_views']."</th><th class=\"text-center\"><a href=\"".htmlspecialchars("?action=viewforum&forumid=".$forumid.$addparam."&sort=".($_GET["sort"] == 'lastpostasc' ? "lastpostdesc" : "lastpostasc"))."\" title=\"".($_GET["sort"] == 'lastpostasc' ? $lang_forums['title_order_post_desc'] : $lang_forums['title_order_post_asc'])."\">".$lang_forums['col_last_post']."</a></th>\n");
		...
		print("</tbody><tfoot><tr><td align=\"left\">\n");
		print("<form class=\"form form-inline\" method=\"get\" action=\"forums.php\"><b>".$lang_forums['text_fast_search']."</b><input type=\"hidden\" name=\"action\" value=\"viewforum\" /><input type=\"hidden\" name=\"forumid\" value=\"".$forumid."\" /><input type=\"text\" class=\"form-control\" name=\"search\" />&nbsp;<input class=\"btn btn-success\" type=\"submit\" value=\"".$lang_forums['text_go']."\" /></form>");
		print("</td>");
?>
...
foreach ($overforums as $a)
{
	...
	print("<thead><tr><th>".htmlspecialchars($overforumname)."</th><th style=\"width:50%;\">&nbsp;</th><th class=\"text-center\">".$lang_forums['col_topics']."</th>" .
	"<th class=\"text-center\">".$lang_forums['col_posts']."</th>" .
	"<th class=\"text-center\">".$lang_forums['col_last_post']."</th><th>".$lang_forums['col_moderator']."</th></tr></thead><tbody>");
	...
	foreach ($forums as $forums_arr)
	{
		...
		print("<tr><td>".$img."</td><td><a href=\"".htmlspecialchars("?action=viewforum&forumid=".$forumid)."\"><b>".$forumname."</b></a>" .$posttoday.
		"<br />".$forumdescription."</td><td class=\"rowfollow\" align=\"center\">".$topiccount."</td><td class=\"rowfollow\" align=\"center\">".$postcount."</td>" .
