...
?>
<thead><tr>
<th class="colhead"><?php echo $lang_topten['col_rank'] ?></th>
<th class="colhead"> <?php echo $lang_topten['col_user'] ?> </th>
<th class="colhead"> <?php echo $lang_topten['col_uploaded'] ?> </th>
<th class="colhead"> <?php echo $lang_topten['col_ul_speed'] ?> </th>
<th class="colhead"> <?php echo $lang_topten['col_downloaded'] ?></th>
<th class="colhead"> <?php echo $lang_topten['col_dl_speed'] ?> </th>
<th class="colhead"> <?php echo $lang_topten['col_ratio'] ?> </th>
<th class="colhead"> <?php echo $lang_topten['col_joined'] ?> </th>
</tr></thead>
<?php
$num = 0;
while ($a = mysql_fetch_assoc($res))
{
	...
	print("<tr><td class=\"rowfollow\">$num</td><td>" . get_username($a["userid"]) .
	"</td><td class=\"rowfollow\">" . mksize($a["uploaded"]) .
	"</td><td class=\"rowfollow\">" . mksize($a["upspeed"]) . "/s" .
	"</td><td class=\"rowfollow\">" . mksize($a["downloaded"]) .
	"</td><td class=\"rowfollow\">" . mksize($a["downspeed"]) . "/s" .
	"</td><td class=\"rowfollow\">" . $ratio .
	"</td><td class=\"rowfollow\">" . gettime($a["added"],true,false). "</td></tr>");
}
end_table();
end_frame();
}
...
<th><?php echo $lang_topten['col_name'] ?></th>
<th><?php echo "<img class=\"snatched\" src=\"pic/trans.gif\" alt=\"snatched\" title=\"".$lang_topten['title_sna']."\" />" ?></th>
<th><?php echo $lang_topten['col_data'] ?></th>
<th><?php echo "<img class=\"seeders\" src=\"pic/trans.gif\" alt=\"seeders\" title=\"".$lang_topten['title_se']."\" />" ?></th>
<th><?php echo "<img class=\"leechers\" src=\"pic/trans.gif\" alt=\"leechers\" title=\"".$lang_topten['title_le']."\" />" ?></th>
<th><?php echo $lang_topten['col_to'] ?></th>
<th><?php echo $lang_topten['col_ratio'] ?></th>
</tr>
</thead>
<?php
$num = 0;
while ($a = mysql_fetch_assoc($res))
{
	...
	$ratio = $lang_topten['text_inf'];
	print("<tr><td>$num</td><td><a href=\"details.php?id=" . $a["id"] . "&amp;hit=1\"><b>" .
	$a["name"] . "</b></a></td><td>" . number_format($a["times_completed"]) .
	"</td><td>" . mksize($a["data"]) . "</td><td>" . number_format($a["seeders"]) .
	"</td><td>" . number_format($a["leechers"]) . "</td><td>" . ($a["leechers"] + $a["seeders"]) .
	"</td><td>$ratio</td>\n");
}
end_table();
end_frame();
...
<thead>
<tr>
<th><?php echo $lang_topten['col_rank'] ?></td>
<th><?php echo $lang_topten['col_country'] ?></td>
<th><?php echo $what?></td>
</tr>
</thead>
<?php
...
while ($a = mysql_fetch_assoc($res))
{
	...
	print("<tr><td>$num</td><td><table border=\"0\" class=\"main\" cellspacing=\"0\" cellpadding=\"0\"><tr><td class=\"embedded\">".
	"<img align=\"center\" src=\"pic/flag/$a[flagpic]\" alt=\"\" /></td><td class=\"embedded\" style='padding-left: 5px'><b>$a[name]</b></td>".
	"</tr></table></td><td>$value</td></tr>\n");
}
end_table();
end_frame();
}
...
function peerstable($res, $frame_caption)
{
	...
	print("<thead><tr><th>".$lang_topten['col_rank']."</td><th>".$lang_topten['col_username']."</td><th>".$lang_topten['col_upload_rate']."</td><th>".$lang_topten['col_download_rate']."</td></tr>");
...
function bonustable($res, $frame_caption)
{
	...
	print("<thead><tr><th>".$lang_topten['col_rank']."</td><th>".$lang_topten['col_username']."</td><th>".$lang_topten['col_bonus']."</td></tr>");

	$n = 1;
	while ($arr = mysql_fetch_assoc($res))
	{
		//die();
		print("<tr><td>$n</td><td>" . get_username($arr["id"]) . "</td><td>" . number_format($arr["seedbonus"], 1) . "</td></tr>\n");
		$n++;
	}
...
function prolinkclicktable($res, $frame_caption)
{
	...
	print("<thead><tr><th>".$lang_topten['col_rank']."</td><th>".$lang_topten['col_username']."</td><th>".$lang_topten['col_clicks']."</td></tr>");

	$n = 1;
	while ($arr = mysql_fetch_assoc($res))
	{
		//die();
		print("<tr><td>$n</td><td>" . get_username($arr["userid"]) . "</td><td>" . number_format($arr["count"]) . "</td></tr>\n");
		$n++;
	}
...
function charityTable($res, $frame_caption)
{
	...
	{
		//die();
		print("<tr><td>$n</td><td>" . get_username($arr["id"]) . "</td><td>" . number_format($arr["charity"]) . "</td></tr>\n");
		$n++;
	}
...
function cmttable($res, $frame_caption, $col2_name)
{
	...
	print("<thead><tr><th>".$lang_topten['col_rank']."</td><th>".$lang_topten['col_username']."</td><th>".$col2_name."</td></tr>");

	$n = 1;
	while ($arr = mysql_fetch_assoc($res))
	{
		print("<tr><td>$n</td><td>" . get_username($arr["userid"]) . "</td><td>" . number_format($arr["num"]) . "</td></tr>\n");
		$n++;
	}
...
function locationtable($res, $frame_caption)
{
	...
	print("<thead><tr><th>".$lang_topten['col_rank']."</th><th>".$lang_topten['col_location']."</th><th>".$lang_topten['col_number']."</th></tr></thead>");

	$n = 1;
	while ($arr = mysql_fetch_assoc($res))
	{
		print("<tr><td>$n</td><td>" . get_username($arr["location_name"]) . "</td><td>" . number_format($arr["num"]) . "</td></tr>\n");
		$n++;
	}
...
function postable($res, $frame_caption)
{
	...
	print("<thead><tr><th>".$lang_topten['col_rank']."</th><th>".$lang_topten['col_username']."</th><th>".$lang_topten['col_topics']."</th><th>".$lang_topten['col_posts']."</th></tr></thead>");

	$n = 1;
	while ($arr = mysql_fetch_assoc($res))
	{
		print("<tr><td>$n</td><td>" . get_username($arr["userid"]) . "</td><td>" . number_format($arr["usertopics"]) . "</td><td>" . number_format($arr["userposts"]) . "</td></tr>\n");
		$n++;
	}
...
function bigtopic_table($res, $frame_caption)
{
	...
	print("<thead><tr><th>".$lang_topten['col_rank']."</td><th>".$lang_topten['col_subject']."</td><th>".$lang_topten['col_posts']."</td></tr></thead>");
	...
	while ($arr = mysql_fetch_assoc($res))
	{
		...
		print("<tr><td>$n</td><td>" . $topic. "</td><td>" . number_format($arr["postnum"]) . "</td></tr>\n");
		$n++;
	}
...
function donortable($res, $frame_caption)
{
	...
	print("<thead><tr><th>".$lang_topten['col_rank']."</th><th>".$lang_topten['col_username']."</th><th>".$lang_topten['col_donated_usd']."</th><th>".$lang_topten['col_donated_cny']."</th></tr></thead>");

	$n = 1;
	while ($arr = mysql_fetch_assoc($res))
	{
		print("<tr><td>$n</td><td>" . get_username($arr["id"]) . "</td><td>" . number_format($arr["donated"], 2) . "</td><td>" . number_format($arr["donated_cny"], 2) . "</td></tr>\n");
		$n++;
	}
...
function clienttable($res, $frame_caption)
{
	...
	print("<thead><tr><th>".$lang_topten['col_rank']."</td><th>".$lang_topten['col_name']."</td><th>".$lang_topten['col_number']."</td></tr>");

	$n = 1;
	while ($arr = mysql_fetch_assoc($res))
	{
		print("<tr><td>$n</td><td>" . $arr["client_name"] . "</td><td>" . number_format($arr["client_num"]) . "</td></tr>\n");
		$n++;
	}
...
function lastsearch_table($res, $frame_caption)
{
	...
	print("<thead><tr><th>".$lang_topten['col_rank']."</td><th>".$lang_topten['col_keyword']."</td><th>".$lang_topten['col_datetime']."</td></tr>");

	$n = 1;
	while ($arr = mysql_fetch_assoc($res))
	{
		print("<tr><td>$n</td><td><a href=\"torrents.php?search=" . rawurlencode($arr["keywords"]) . "\">" . $arr["keywords"] . "</a></td><td>" . gettime($arr["adddate"], true,false) . "</td></tr>\n");
		$n++;
	}
...
function search_ranktable($res, $frame_caption)
{
	...
	print("<thead><tr><th>".$lang_topten['col_rank']."</td><th>".$lang_topten['col_keyword']."</td><th>".$lang_topten['col_times']."</td></tr>");

	$n = 1;
	while ($arr = mysql_fetch_assoc($res))
	{
		print("<tr><td>$n</td><td><a href=\"torrents.php?search=" . rawurlencode($arr["keywords"]) . "\">" . $arr["keywords"] . "</a></td><td>" . number_format($arr["count"]) . "</td></tr>\n");
		$n++;
	}
...
<tr>
<th><?php echo $lang_topten['col_rank'] ?></td>
<th> <?php echo $lang_topten['col_user'] ?> </td>
<th> <?php echo $lang_topten['col_torrent_uploaded'] ?> </td>
<th> <?php echo $lang_topten['col_uploaded'] ?> </td>
<th> <?php echo $lang_topten['col_torrent_downloaded'] ?> </td>
<th> <?php echo $lang_topten['col_downloaded'] ?></td>
<th> <?php echo $lang_topten['col_ratio'] ?> </td>
<th> <?php echo $lang_topten['col_joined'] ?> </td>
</tr>
</thead>
<?php
$num = 0;
while ($a = mysql_fetch_assoc($res))
{
	...
	$ratio = $lang_topten['text_inf'];
	print("<tr><td>$num</td><td>" . get_username($a["userid"]) .
	"</td><td>" . number_format($a["supplied"]) .
	"</td><td>" . mksize($a["uploaded"]) .
	"</td><td>" . number_format($a["snatched"]) .
	"</td><td>" . mksize($a["downloaded"]) .
	"</td><td>" . $ratio .
	"</td><td>" . gettime($a["added"]). "</td></tr>");
}
end_table();
end_frame();
}
...
function stylesheettable($res, $frame_caption)
{
	...
	print("<thead><tr><th>".$lang_topten['col_rank']."</td><th>".$lang_topten['col_name']."</td><th>".$lang_topten['col_number']."</td></tr></thead>");

	$n = 1;
	while ($arr = mysql_fetch_assoc($res))
	{
		//die();
		print("<tr><td>$n</td><td>" . $arr["stylesheet_name"] . "</td><td>" . number_format($arr["stylesheet_num"]) . "</td></tr>\n");
		$n++;
	}
...
function languagetable($res, $frame_caption)
{
	...
	{
		//die();
		print("<tr><td>$n</td><td>" . $arr["lang_name"] . "</td><td>" . number_format($arr["lang_num"]) . "</td></tr>\n");
		$n++;
	}
